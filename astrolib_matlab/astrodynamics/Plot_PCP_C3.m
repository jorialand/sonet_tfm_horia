function [  ] = Plot_PCP_C3 ( tofs, departure_dates, c3,...
    departure_planet, arrival_planet, delta_v_title_prefix, ...
    file_name, label_x, label_y, z_cap, fopts )
%PLOT_PCP_C3 plots a Porkchop plot with C3 values
...
% Plot Data
contourf(tofs,departure_dates,c3,256,'LineStyle','none'); % Contourf
if fopts.doCtrs % Draw contour lines/labels
    ctrs = [0,5,10,20:20:120,150,200:100:1000]; % Contours
    [C,~] = contour(tofs,departure_dates,c3,ctrs,'Color','w'); % Contours
...
% Title
title(['PCP | ',planets_str,delta_v_title_prefix,' C_3 [km^2/s^2]']);
