function [ c3d, c3a, c3bpm, dvd, dva, ...
    dvbpm, dvt, theta ] = PCP_Grid_BPMA ( ...
...
c3bpm = zeros(ndd,ntf); % C3 BPM velocity
...
for i=1:ndd
    ...
    for j=1:ntf
        ...
        [ c3d(i,j), c3a(i,j), c3bpm(i,j), ~, ~, ~, ~, theta(i,j) ] = ...
...
dvbpm = sqrt(c3bpm); % BPM DeltaV
dvt = dvd + dva + dvbpm; % Total DeltaV
