function [ c3d, c3a, c3bpm, dvd, dva, dvbpm, dvt, theta ] = ...
    InterplanetaryTransfer_Lambert_BPMA ( ...
    departure_planet, arrival_planet, departure_date, tof, mr, lp )
%PCP_GRID Pork-chop plot
%   Returns the results from a Pork-chop Plot via Lambert arcs + auto BPM
%
% Inputs:
...
alpha = 0.4; % Broken plane manoeuvre location
beta = 30; % Span of degrees from 180deg transfer
...
% Select auto BPM manoeuvre
if abs(theta-pi) > beta*pi/180 % No BPM required

    % Select long-way transfer (Type II) if required
    if theta > pi, lw = 1; else, lw = 0; end

    % Solve Lambert problem between P1 and P2
    [vsd,vsa] = Lambert(r1,r2,tof*days2secs,mu,lw,0,lp);
    
    % No BPM manoeuvre
    vsa_bpm = 0;
    vsd_bpm = 0;

else % We are near 180deg transfer; do BPM manoeuvre

    % Intersect arrival planet with departure planet's orbital plane
    ...
    [vsd,~] = Lambert(r1,r2_inc1,tof*days2secs,mu,lw,0,0);
